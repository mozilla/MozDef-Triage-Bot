...
a [Redis](https://redis.io) instance containing information about jobs being
processed.  Most activity will be triggered by events from an
...
The Redis instance will be treated as a hash-table store whose contents will be
typed according to the `Redis` type defined in
...
alert description for processing.  This function has the following
responsibilities.

1. If there are no jobs for the user identified, store a new `UserRecord`.
2. If a job is already recorded for the alert in question (identified by its
`AlertLabel`) then it will *not* be updated so that a user does not receive
multiple messages for the same kind of alert.
3. If a job does not exist, a new `AlertProcessingRecord` will be stored.

All new jobs will have a [ttl](https://redis.io/commands/ttl) set to trigger
the Job Timeout function to handle cleanup.
...
1. Update the `response` field of the `AlertProcessingRecord`.

#### Notify User

The Notify User lambda function will be invoked in response to a new job being
appeneded for a user by the Alert Notification function.  It has the following
responsibilities.

1. Invoke the Slack API to
...
1. Send a `UserResponseMessage` to the SQS worker containing information about
the alert message that was responded to.

#### Job Timeout

The Job Timeout lambda function will be invoked when an `AlertProcessingRecord`
has its timeout exceeded.  This function will carry out the following.

1. If the user has *not* responded to a message about the alert in question,
send a `UserResponseMessage` with a `response` of `"none"` to the SQS worker.
...
The web server component will utilize a data store like
[Redis](https://redis.io/) to record information about alerts that it is
processing so that it can make decisions about when to message users and
what information to send back to MozDef.  Information in this data store will
be represented using the following data structure.

```typescript
type AlertProcessingRecord = {
  // The request originally received.
...
In a data store such as Redis, multiple of these records will be maintained for
each user.  These records will be collected under a hierarchical as follows.
...
// Top-level Redis data structure maps a user's email to the records describing
...
retrieve relevant information from Redis given a single `AlertTriageRequest`.
